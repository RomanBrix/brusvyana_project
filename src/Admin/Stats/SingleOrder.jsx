...
export default function SingleOrder() {
    ...
    useEffect(() => {
        if (order?.notes.length > 0) {
            const ids = order.notes
                .map((note) => note.author)
                .filter((i) => i !== "UkrPochta_Bot");
            console.log(ids);
            adminRequestUser
                .get(`/users/usernames`, { params: { ids } })
                .then((res) => {
                    // console.log(res)
                    const { data } = res;
                    setOrder((prev) => ({
        ...
        <div className="oneOrder">
            <div className="whiteBg order-info">
                ...
                <div className="status">
                    Статус:{" "}
                    <span className={`tag ${statusFlag(order.status).color}`}>
                        {statusFlag(order.status).text}
                    </span>
                    <ul
                        className="list"
                        onClick={(e) => {
                    ...
                    >
                        <li data-value="done">
                            <span className={`tag blue`}>Виконано</span>
                        </li>
                        <li data-value="payed">
                            <span className={`tag green`}>Оплачено</span>
                        </li>
                        <li data-value="new">
                            <span className={`tag orange`}>Новий</span>
                        </li>
                        <li data-value="none">
                            <span className={`tag red`}>Не визначено</span>
                        </li>
                    </ul>
                </div>
    ...
    function deleteNote(noteId) {
        ...
        adminRequestRetail
            .put(`/orders/order/${order._id}`, { notes: newNotes })
            .then((res) => {
                setOrder((prev) => ({
                    ...prev,
    ...
    function changeStatus({ target }) {
        ...
        console.log(status);
        adminRequestRetail
            .put(`/orders/order/${order._id}`, { status })
            .then((res) => {
                setOrder((prev) => ({
                    ...prev,
    ...
    function addNote() {
        ...
        adminRequestRetail
            .post(`/orders/note`, { note: createNote, id: order._id })
            .then((res) => {
                setOrder(res.data);
                setNote("");
            })
    ...
    function renderUser() {
        return <div className="guest user">{/* ПОКА ПОХУЮ */}</div>;
    }
    function renderGuestUser() {
        return (
            <div className="guest">
                <div className="user-tag">
                    <span className="tag orange">Гість</span>{" "}
                </div>
                <div className="block name">
                    <span className="material-icons">account_circle</span>{" "}
                    {order.guestUser.secondName} {order.guestUser.name}
                </div>
        ...
        return products.map((product, index) => {
            return (
                <div className="product" key={index}>
                    ...
                    {product.varTitle ? (
                        <p>
                            Варiант: <b>{product.varTitle}</b>{" "}
                        </p>
                    ) : null}
                    {/* <div className="price">{product.price} UAH</div> */}
                    <div className="count">
                        кількість: <b>{product.quantity}</b> шт.
                    </div>
                </div>
            );
        });
